...
export function ConditionBuilder({ columns, onSave, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="bg-card border border-border rounded-xl shadow-2xl w-full max-w-xl" onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div>
            <h3 className="font-semibold text-sm">Define Workflow Rule</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Set conditions and resulting actions for data generation.</p>
          </div>
        ...
        <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
          {/* Rule name */}
          <input
            className="w-full bg-background border border-border rounded-md px-3 py-1.5 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-primary"
            ...
            <div className="bg-background border border-border rounded-lg p-3 space-y-2">
              {conditions.map((cond, i) => (
                <div key={i} className="space-y-1.5">
                  {i > 0 && (
                    <div className="text-center text-xs text-muted-foreground font-medium py-1">AND</div>
                  )}
                  <div className="flex items-center gap-2">
                    <select
                      className="flex-1 bg-card border border-border rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                      value={cond.column}
                    ...
                    <select
                      className="w-28 bg-card border border-border rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                      value={cond.op}
          ...
          <div>
            ...
            <div className="bg-background border border-border rounded-lg p-3 space-y-3">
              ...
              {actionMeta.hasValue && (
                <input
                  className="w-full bg-card border border-border rounded px-2 py-1.5 text-xs font-mono focus:outline-none focus:ring-1 focus:ring-primary"
          ...
          <div>
            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Preview — Affected Columns</p>
            <div className="flex gap-2 flex-wrap">
              {conditions.filter(c => c.column).map((c, i) => (
                <div key={i} className="bg-card border border-primary/30 rounded-md px-3 py-2 text-xs">
                  <div className="text-muted-foreground font-mono mb-0.5">
              ...
              {actionColumn && (
                <div className="bg-card border border-green-500/30 rounded-md px-3 py-2 text-xs">
                  <div className="text-muted-foreground font-mono mb-0.5">
